...
if (!fs.existsSync(process.env.SERVER_KEY)) {
  throw new Error(
    `Key file "${process.env.SERVER_KEY} could not be found from ${process.cwd()}"`
  );
}

export const serverPublicKey = fs.readFileSync(process.env.SERVER_KEY).toString();

// Check the key is a valid key in .PEM format
try {
  crypto.publicEncrypt(serverPublicKey, Buffer.from("Test"));
} catch (e) {
  const errorMessage = `Key in file ${process.env.SERVER_KEY} was not a valid public key in PEM format`;
